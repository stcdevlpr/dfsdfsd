    ...
    while True:
        async with aiohttp.ClientSession() as s:
            async with s.get(f"http://{HOST}/Accs") as r:
                ...
                print(myAccs)
        for acc in myAccs:
            if acc[2] not in states: tasks.append(asyncio.create_task(approve_and_welcome_users(
                Client(f"./sess/{acc[2]}", acc[0], acc[1], phone_number=acc[2], password=acc[3]), acc[-1])))
