...
bot = Bot(token='7694527778:AAHdVpISeDf1jyOyFaYc78MaIkmtfJAx7UU',
...
message_text = (f"<b>Как работать с ботом?</b>\n\n"
                    f"<b>🔹 Получение API_ID и HASH_ID</b>\n"
                    f"Получите ключи <b>API_ID</b> и <b>HASH_ID</b> на сайте <a href='https://my.telegram.org/auth?to=apps'>my.telegram.org</a>. "
                    f"Если аккаунт, от которого получены ключи, будет удалён, подключение всех связанных аккаунтов станет невозможным.\n\n"
                    f"<b>🔹 Добавление администраторов и ключей</b>\n"
                    f"Администраторы, <b>API_ID</b> и <b>HASH_ID</b> добавляются вручную.\n\n"
                    f"<b>🔹 Добавление аккаунта</b>\n"
                    f"В разделе <i>Добавить аккаунт</i> происходит авторизация и получение <b>файла сессии</b>. Файл автоматически сохраняется "
                    f"в базе и отображается в разделе <i>Мои аккаунты</i>.\n"
                    f"<i>✏️ Рекомендация:</i> Запишите уникальный номер сессии для удобства.\n\n"
                    f"<b>🔹 Раздел 'Мои аккаунты'</b>\n"
                    f"В этом разделе отображается список ID ваших аккаунтов. Нажав на ID, вы перейдёте в меню настроек выбранного аккаунта.\n\n"
                    f"<b>✅ Следуйте этим шагам для эффективной работы с ботом.</b>")
...
async def startmsg(message: Message):
    ...
    await message.answer(text=message_text, parse_mode="HTML", reply_markup=startKB)
...
async def addingTGphone(message: Message, state: FSMContext):
    ...
    try:
        await message.delete();await bot.delete_message(message.chat.id, message.message_id - 1)
    except:
        pass
    try:
    ...
    db[message.text]['app'] = Client(f"./sess/{message.text}", api_id=apiid, api_hash=apihash,
...
async def addingTGCOde(message: Message, state: FSMContext):
    ...
    while True:
        try:
            ...
            await ErrorConnectAcc(message, 'false code')
        except PasswordHashInvalid:
            await ErrorConnectAcc(message, "false pass hash")
...
async def endingConnection(message: Message, state: FSMContext):
    ...
    async with aiohttp.ClientSession() as s:
        async with s.post(
                f"http://{HOST}/addAcc?phone={client.phone_number}&api_id={client.api_id}&api_hash={client.api_hash}&twofa={client.password}&channelID={message.text}&defaultInterval=30-60") as r:
            print(await r.json())
    try:
        await message.delete();await bot.delete_message(message.chat.id, message.message_id - 1)
    except:
        pass
    await bot.send_message(message.from_user.id, f"Аккаунт Успешно подключен!", reply_markup=startKB,
                           parse_mode=ParseMode.HTML)
    try:
        await bot.send_document(chat_id=message.from_user.id, document=FSInputFile(f"{client.name}.session"),
                                caption=client.name.split('/')[-1])
    except Exception as e:
        print(e)
...
async def myAccs(callback: CallbackQuery):
    ...
    if len(myAccs) > 0:
        ...
        await callback.message.edit_text(text=callback.message.text,
                                         reply_markup=InlineKeyboardMarkup(inline_keyboard=inlinekb))
    else:
...
async def getAcc(callback: CallbackQuery):
    ...
    try:
        async with Client(f"./sess/{accData[2]}", accData[0], accData[1], phone_number=accData[2],
                          password=accData[3]) as c:
            lastTgmsg = ''.join([m for m in [x async for x in c.get_chat_history(777000, 1)][0].text.split() if
                                 m[:-1].isdigit() and len(m) == 6])
            if lastTgmsg != '':
                await callback.answer(f"Telegram Code: {lastTgmsg}")
            else:
                await callback.answer('💚')
        ...
        await callback.message.edit_text(await getAccText(callback.message, accData[2]),
...
async def delAcc(callback: CallbackQuery):
    async with aiohttp.ClientSession() as s:
        async with s.post(f"http://{HOST}/delAcc?phone={callback.data.split('_')[-1]}") as r:
            if r.status != 200: await callback.answer('Ошибка при отправке запроса')
            if (await r.json()): await callback.answer();await callback.message.edit_text('Аккаунт удален',
...
async def updateInter(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "Введите новый интервал проверки новых пользователей в секундах через '-'\nПример: 30-90",
...
async def gettingIntervalValue(message: Message, state: FSMContext):
    ...
    async with aiohttp.ClientSession() as s:
        async with s.post(f"http://{HOST}/addInter?inter={message.text}&phone={phone}") as r:
            ...
            if await r.json():
                await message.answer(await getAccText(message, phone), reply_markup=InlineKeyboardMarkup(
                    inline_keyboard=[
                        [InlineKeyboardButton(text="Изменить интервал", callback_data=f"updateInterval_{phone}")],
...
async def dodelTxt(callback: CallbackQuery):
    async with aiohttp.ClientSession() as s:
        async with s.post(
                f"http://{HOST}/delText?notif={callback.data.split('_')[-1]}&phone={callback.data.split('_')[-2]}") as r:
...
async def createNewText(message: Message, state: FSMContext):
    ...
    try:
        ...
        pass
    await bot.send_message(message.from_user.id, await getAccText(message, phone), reply_markup=InlineKeyboardMarkup(
        inline_keyboard=[[InlineKeyboardButton(text="Изменить интервал", callback_data=f"updateInterval_{phone}")],
                         [InlineKeyboardButton(text="Текста", callback_data=f"myTexts_{phone}")],
                         [InlineKeyboardButton(text="Изменить канал", callback_data=f"changeChannel_{phone}")],
                         [InlineKeyboardButton(text='Выход', callback_data=f'cancel')]
                         ]))
